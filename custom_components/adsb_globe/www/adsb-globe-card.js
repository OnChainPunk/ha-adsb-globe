/* ADS-B Globe Lovelace card — tar1090-style map for Home Assistant.
 * Polls at 1s (same as tar1090 / ADS-B Exchange globe) for the current view.
 */
...
function loadScript(src) {
  return new Promise((resolve, reject) => {
    if ([...document.scripts].some((s) => s.src.includes("leaflet.js") && s.src.includes("adsb_globe"))) {
...
function loadCss(href) {
  if ([...document.styleSheets].some((s) => s.href && s.href.includes("leaflet.css"))) return;
...
class AdsbGlobeCard extends HTMLElement {
  ...
  async _build() {
    ...
    loadCss("/adsb_globe/leaflet.css");
    try {
      await loadScript("/adsb_globe/leaflet.js");
    } catch (err) {
  ...
  async _fetch() {
    if (this._gone || this._paused || !this._hass || this._busy) return;
    this._busy = true;
    const c = this._map.getCenter();
    const dist = this._viewDist();
    try {
      const data = await this._hass.callApi("GET", `adsb_globe/aircraft?lat=${c.lat.toFixed(4)}&lon=${c.lng.toFixed(4)}&dist=${dist}`);
      this._paint(data);
    } catch (err) {
...
window.customCards.push({
  ...
  name: "ADS-B Globe",
  description: "Live tar1090-style aircraft map. Polls the current view at 1 second.",
  preview: true,
});
